...
| **Embedding Model** | Layer 2 | Ubuntu GPU Server | 8001 | Pre-retrieval: Text → Vector conversion |
| **Vector Store (In-Memory)** | Layer 2 | Mac (Flask process) | - | Retrieval: Store/search vectors (not scalable) |
| **vLLM Server** | Layer 4 | Ubuntu GPU Server | 8000 | Generation: High-performance LLM inference |
| **Milvus Vector DB** | Layer 4 | Ubuntu GPU Server | 19530 | Retrieval: Production vector database |
| **Reranker Model** | Layer 7 | Ubuntu GPU Server | - | Post-retrieval: Rescoring retrieved chunks |
...
│ • Git repository                                        │
│                                                          │
│ Role: Development, coordination, lightweight services   │
└────────────────────┬────────────────────────────────────┘
                     │
                     │ Tailscale VPN (100.64.x.x)
                     │
┌────────────────────┴────────────────────────────────────┐
│ Ubuntu GPU Server (24/7 - School)                       │
│ ────────────────────────────────────                    │
│ Hardware: i7-12700, RTX 4080 16GB, 64GB RAM            │
│                                                          │
│ ML Models:                                              │
│ • Embedding Server (8001) - nomic-embed/bge-large      │
│ • vLLM (8000) - Llama/Mistral/Qwen                     │
│ • Milvus (19530) - Vector database (GPU-accelerated)   │
│ • Reranker (optional) - Cross-encoder models           │
│                                                          │
│ Storage:                                                │
│ • ~/thesis-data/documents/ - Raw document files        │
│                                                          │
│ Role: GPU inference, vector search, document storage    │
└─────────────────────────────────────────────────────────┘
```
...
- **Examples**: `nomic-embed-text`, `bge-large-en-v1.5`, `e5-mistral-7b-instruct`
- **Input**: Text (query or document chunk)
- **Output**: Dense vector (768 or 1024 dimensions)
- **Role in RAG**: **Pre-retrieval** - Converts text to semantic representation
- **Deployment**: Ubuntu GPU Server (Port 8001)
- **Why Separate**:
  ...
  - Smaller model (~400MB vs 7GB+ for LLM)
- **Performance**: ~1000 embeddings/second on RTX 4080
...
- **Models**: Llama-3-8B, Mistral-7B, Qwen-7B, etc.
- **Role in RAG**: **Generation** - Produces final answer from context
- **Deployment**: Ubuntu GPU Server (Port 8000)
...
- **Performance**: RTX 4080 can run 7B models at ~50 tokens/sec
...
- **Deployment**: Ubuntu GPU Server (Port 19530)
- **Why Milvus over alternatives**:
  - **GPU-accelerated search**: Uses RTX 4080 for vector operations
...
# ===== MILVUS SETUP (Ubuntu:19530) =====
connections.connect("default", host="100.64.x.1", port="19530")
...
# Insert chunks
for chunk in all_chunks:
    # Get embedding from Ubuntu:8001
    embedding_response = requests.post(
        "http://100.64.x.1:8001/embed",
...
# ===== QUERY WITH vLLM (Ubuntu:8000) =====
def query_rag(user_question):
    # 1. Embed query (Ubuntu:8001)
    query_embedding = requests.post(
        "http://100.64.x.1:8001/embed",
        json={"text": user_question}
    ).json()["embedding"]

    # 2. Search Milvus (Ubuntu:19530)
    ...
    # 4. Generate with vLLM (Ubuntu:8000)
    response = requests.post(
        "http://100.64.x.1:8000/v1/completions",
        json={
            "model": "mistral-7b",
            "prompt": prompt,
            "max_tokens": 500,
