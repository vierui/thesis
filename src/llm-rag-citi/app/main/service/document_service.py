...
async def extract_document_metadata(file_path: str, file_tag: str):
    """
    Extract metadata (title, topics, confidence) from a document file.

    Args:
        file_path: Path to the document file
        file_tag: File type/extension (e.g., 'pdf', 'docx')

    Returns:
        dict: Metadata containing title, topics, and confidence scores
    """
    if not check_document_validation(file_tag):
        raise HTTPRequestException(message=f"File type '{file_tag}' is not supported", status_code=400)

    # For now, only support PDF
    if file_tag != 'pdf':
        raise HTTPRequestException(message="Currently only PDF files are supported", status_code=400)

    try:
        # Extract text sample from PDF
        text_sample = extract_pdf_text_sample(file_path, max_pages=3, max_chars=5000)

        if not text_sample or len(text_sample.strip()) < 50:
            raise HTTPRequestException(
                message="Insufficient text content in document to extract metadata",
                status_code=400
            )

        # Create prompt with the extracted text
        prompt = PromptTemplate(DOCUMENT_METADATA_PROMPT_TEMPLATE)
        formatted_prompt = prompt.format(content=text_sample)

        # Call LLM to extract metadata
        response = await generation_llm.acomplete(formatted_prompt)
        ...
        try:
            ...
            required_fields = ["title", "topics", "confidence"]
            if not all(field in metadata for field in required_fields):
                raise ValueError("Missing required fields in LLM response")

            if not isinstance(metadata["topics"], list) or len(metadata["topics"]) == 0:
        ...
        raise HTTPRequestException(
            message=f"Failed to extract document metadata: {str(e)}",
            status_code=500
        )
