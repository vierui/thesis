...
- Generator inference :
  ...
  A system where prefill dominates (like RAG) prefers a different hardware design than one where decoding dominates (Chap 1.3)

- the agent's *control flow*—its reasoning, planning, and orchestration—is itself a primary, and highly optimizable, source of computational cost and latency, distinct from the cost of the LLM calls it makes. (Chap 2.1)

- thesis focused on optimizing the hardware for an *agentic RAG system* that *only* optimizes the LLM/GPU component (e.g., with a new quantization method) is ignoring 90% of the latency problem. dominant *CPU-side bottleneck*: the agent's control flow and its RAG tool-use. (Chap 2.2)

- a dynamic retriever is not 'run-time' optimized since ressources / features will be called on demand and might trigger a complex, multi-step chain (Chap 3.1)
  *e.g. CPU-logic \-\> GPU-reranker \-\> CPU-logic \-\> web-tool-I/O \-\> CPU-logic \-\> GPU-LLM.* 

- Agents for structured *knowledge graphs* (multi-hop reasoning by building an entity-relationship graph from the knowledge base and reasoning over it) is HEAVY for GPU, CPU and Graph size. (Chap 3.2)

- RAG Knowledge != Agentic memory/context
  → Improvement by combining them ? 
  → 

- Primary trade-off of Agentic RAG is quality for *time and money* : Agentic RAG involves "more LLM calls \= more tokens \= higher cost... or more load on your infrastructure"(Chap 3.3)
  
  not a parallelizable problem. The agentic loops 49 and reflection steps 39 are *serial*: Action 1 \-\> Reflection 1 \-\> Action 2 \-\>.... This creates an additive latency.  **true bottleneck is often hidden in the "lower stack" infrastructure: memory, coordination, and retrieval latency** (Chap 4.1)

- In Agentic RAG, the system is bottlenecked by **CPU-bound agentic control flow (90.6% latency, 44% energy)**, which *includes* the RAG retrieval task (Chap 4.2)
  
  Reason : Unpredictable Workload - as mentionned in Chap 3.1
  Optimization required : **CPU-GPU Synchronization & CPU Tool Execution**

- Frameworks for "limited hardware resources" exist, but aren't yet designed for agentic workflows.61 The literature frames "agentic AI on mobile/edge" as a future goal, not a current reality, precisely *because* of these resource constraints.56 -- research opportunity. (Chap 4.3)

- Software layer optimizations on the generator (chap 5.1): 
	- Via compression : 
		- **Quantization** (reducing weigths precision) 
		- **Pruning and Distillation** (cutting weights or teaching a student model)

	- Inference improvement : 
		- **Speculative Decoding** : a small model runs tokens prediction very fast and and larger model computes the log-probability over the whole sequence that was generated. 
		  ...
		  The small model is often right on _easy_ tokens (“The”, “and”, “of”, “is”) but it’s often wrong on _semantic_ or _knowledge-heavy_ ones
		  
		  GPU utilization is smoother and memory reuse (KV cache) is more efficient → energy per output token (_J/token_) goes down.

		- **Efficient Attention** : aim to solve the prefill bottleneck (avoid large intermediate matrices in GPU memory)

- Hardware-Layer Optimizations (Chap 5.2)
	- LLM Accelerators : FPGA/ASIC 
	- CPU: SIMD instructions for cpus (outperforms vector search on on standard x86 CPUs)
...
- Current AI systems are overwhelmingly designed for *maximum performance*.88 Energy optimization is treated as a secondary 
  →  agent that *autonomously* manages its *own* energy consumption as a *primary goal*. This requires the agent to perceive and reason about *energy* as a finite resource

- hardware acceleration is fragmented, most approach ignores the *dominant system-level bottleneck*: the *CPU-bound agentic loop* that constitutes 90.6% of latency
...
# standing papers
## [RAGO: Systematic Performance Optimization for Retrieval-Augmented Generation Serving](/Users/rvieira/Documents/Master/thesis/0-docs/implementation/100-literature/101-research/papers/2503_rago-systematic-performance-optimiz-rag.pdf)
### source
Google, ETH, Google DeepMind
### date
march 2025
### keywords
...
## [Difficulty-Aware Agent Orchestration in LLM-Powered Workflows](/Users/rvieira/Documents/Master/thesis/0-docs/implementation/100-literature/101-research/papers/2509_difficulty-aware-agent-orchestration.pdf])
### source
